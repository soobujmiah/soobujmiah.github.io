...
`.repo/project.yaml` and `.repo/STATUS.md` are machine-generated by `tools/repo_knowledge/`
(vendored from canonical `soobujmiah/skb`, see `tools/repo_knowledge/README.md` and
`soobujmiah/skb` → `governance/DETERMINISTIC_STATE_SYNC_POLICY.md`), kept current by
`.github/workflows/repo-knowledge-sync.yml` on every push to `main`. `build.status` reflects
`lint` + `typecheck` + `build` (which itself runs every gate in the table above via `prebuild`);
`test.status` reflects `check:build`, the closest thing this project has to a post-build
...
- `components/WorldMap.tsx` — the hero environment: a dark, deep-green,
  **page-aware** global map (Natural Earth 1:110m contours + per-country
  ...
  the seven sections own one deterministic geographic focus (`app/geo.ts` →
  ...
  reduced motion.
- `components/sections.tsx` — the seven curated pages (Hero → Contact).
  Technical work combines the former overview and stack. Work includes the
  featured spotlight and selected repositories (curated subset + GitHub route),
...
The former `/stack/` and `/open-source/` URLs still load and canonicalize to
`/presence/` and `/work/` (with equivalent Bengali URLs).
...
app/
  ...
  (en)/[section]/page.tsx six English section routes + two legacy aliases
...
components/
  ...
  sections.tsx        the seven curated pages (spotlight, carousels, accordion)

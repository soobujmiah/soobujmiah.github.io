...
- `components/WorldMap.tsx` — the hero environment: a dark, deep-green,
  **page-aware** global map (Natural Earth 1:110m contours, Miller-projected,
  generated by `tools/make-worldmap.py`). Each of the nine sections owns one
  deterministic geographic focus (`app/geo.ts` → `GEO_FOCUS`, Home =
  Bangladesh); the camera flies the SVG viewBox between them on page turns.
  A soft focus glow follows the current geography, a handful of SMIL packets
  travel the origin→hub arcs, and a static technical-motif layer (circuits,
  orbits, waveforms) sits behind the content. No grid, no tiles, no boxes,
  no light layer; zero canvas, zero WebGL; paused when hidden, snapping and
  packet-free under reduced motion.

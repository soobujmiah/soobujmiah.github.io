...
/* ═══════════════════════════════════════════════════════════════
   ...
   Green + dark theme. Content-aware pacing. All existing content,
  ...
  return (
    <div ref={ref} className="scene-fill">
      ...
      >
        ...
        >
          <span className="font-mono text-[9px] uppercase tracking-[0.2em]" style={{ color: 'rgba(228,226,223,0.2)' }}>
            Scroll
          </span>
          ...
          {presence.map((p, i) => (
            <Reveal key={p.label} delay={i * 0.08}>
              <div className="text-center">
                ...
                <p className="mt-1.5 font-mono text-[10px] leading-relaxed" style={{ color: 'rgba(228,226,223,0.35)' }}>
                  {p.detail}
                </p>
              </div>
            </Reveal>
          ...
          <div className="space-y-4">
            {[
              ...
              <Reveal key={i} delay={0.1 + i * 0.06}>
                <p className="text-sm leading-[1.8]" style={{ color: 'rgba(228,226,223,0.45)' }}>{p}</p>
              </Reveal>
            ))}
          </div>
          ...
          <Reveal delay={0.15}>
            <div className="rounded-xl p-6 space-y-0" style={{ border: '1px solid rgba(228,226,223,0.05)', background: 'rgba(255,255,255,0.01)' }}>
              {[
                ...
                <div key={f.label} className={`flex items-start justify-between gap-4 py-3 ${i < 3 ? 'border-b' : ''}`} style={{ borderColor: 'rgba(228,226,223,0.04)' }}>
                  <span className="font-mono text-[10px] uppercase tracking-wider" style={{ color: 'rgba(228,226,223,0.25)' }}>{f.label}</span>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            <div className="flex items-center gap-3 mb-1.5">
              <span className="font-mono text-[10px]" style={{ color: 'rgba(228,226,223,0.2)' }}>0{index + 1}</span>
              <span className="font-mono text-[10px]" style={{ color: 'rgba(228,226,223,0.2)' }}>{project.year}</span>
            </div>
          ...
          <Magnetic
            ...
            style={{ border: '1px solid rgba(228,226,223,0.08)', color: 'rgba(228,226,223,0.35)' }}
          >
        ...
        <p className="text-xs leading-[1.7] mb-4" style={{ color: 'rgba(228,226,223,0.4)' }}>
          {project.description}
        </p>

        <p className="text-xs leading-[1.7] mb-5" style={{ color: 'rgba(228,226,223,0.3)' }}>
          <span style={{ color: 'rgba(228,226,223,0.5)' }}>Evidence: </span>{project.evidence}
        </p>

        <div className="flex flex-wrap gap-1.5">
          {project.topics.map((t) => (
            <span key={t} className="rounded-full px-2.5 py-0.5 font-mono text-[10px]" style={{ border: '1px solid rgba(228,226,223,0.06)', color: 'rgba(228,226,223,0.3)' }}>
              {t}
            </span>
          ))}
        </div>
            ...
            <Reveal key={r.title} delay={0.08 + i * 0.06}>
              ...
              >
                <div className="flex items-center gap-2.5 mb-3">
                  ...
                  />
                  <span className="font-mono text-[9px] uppercase tracking-[0.15em]" style={{ color: 'rgba(228,226,223,0.25)' }}>
                    {r.status}
                  </span>
                </div>
                <h3 className="text-base font-semibold mb-1.5" style={{ color: '#e4e2df' }}>{r.title}</h3>
                <p className="text-xs leading-[1.7]" style={{ color: 'rgba(228,226,223,0.4)' }}>{r.description}</p>
              </div>
            </Reveal>
          ))}
        </div>
            ...
            <Reveal key={d.name} delay={0.06 + i * 0.05}>
              <div className="rounded-xl p-5 h-full" style={{ border: '1px solid rgba(228,226,223,0.04)', background: 'rgba(255,255,255,0.008)' }}>
                ...
                <ul className="space-y-1.5">
                  {d.items.map((item) => (
                    <li key={item} className="text-xs" style={{ color: 'rgba(228,226,223,0.35)' }}>{item}</li>
                  ))}
                </ul>
              </div>
            </Reveal>
            ...
            <Reveal key={r.name} delay={0.03 + i * 0.03}>
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {r.stars > 0 && <span className="font-mono text-[9px]" style={{ color: 'rgba(228,226,223,0.2)' }}>★ {r.stars}</span>}
                </div>
                <p className="flex-1 text-[11px] leading-relaxed mb-3" style={{ color: 'rgba(228,226,223,0.3)' }}>{r.desc}</p>
                <div className="flex items-center gap-2">
                  <span className="h-2 w-2 rounded-full" style={{ background: '#22c55e', opacity: 0.6 }} />
                  <span className="font-mono text-[10px]" style={{ color: 'rgba(228,226,223,0.25)' }}>{r.lang}</span>
                </div>
              </Magnetic>
            </Reveal>
          ))}
    ...
    <div className="scene-fill">
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-5xl flex-col justify-center px-6 py-20">
        ...
        <div className="relative">
          <div className="absolute left-[5px] top-0 bottom-0 w-px" style={{ background: 'rgba(228,226,223,0.05)' }} />
          {workHistory.map((w, i) => (
            <Reveal key={i} delay={0.06 + i * 0.05}>
              <div className="relative pl-8 pb-6 last:pb-0">
                <div className="absolute left-0 top-1 h-3 w-3 rounded-full border-2" style={{ borderColor: '#22c55e', background: '#060608' }} />
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-1">
                  <span className="font-mono text-[10px]" style={{ color: 'rgba(228,226,223,0.25)' }}>{w.period}</span>
                  <h3 className="text-sm font-semibold" style={{ color: '#e4e2df' }}>{w.role}</h3>
                </div>
                <p className="text-xs" style={{ color: 'rgba(228,226,223,0.4)' }}>
                  {w.company} · {w.location}
                </p>
                <p className="text-xs mt-1 leading-relaxed" style={{ color: 'rgba(228,226,223,0.3)' }}>{w.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
  ...
  return (
    <div className="scene-fill">
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-3xl flex-col items-center justify-center px-6 text-center">
        ...
        <Reveal delay={0.12}>
          <p className="mx-auto max-w-md mb-10 text-sm" style={{ color: 'rgba(228,226,223,0.4)' }}>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {contactChannels.map((c) => (
              ...
              >
                ...
                <span className="font-mono text-[10px]" style={{ color: 'rgba(228,226,223,0.25)' }}>{c.value}</span>
              </Magnetic>
            ))}
          </div>
        </Reveal>
